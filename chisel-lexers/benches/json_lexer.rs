...
fn benchmark_canada(c: &mut Criterion) {
    c.bench_function("JSON lex of canada", |b| b.iter(canada));
}
fn benchmark_citm_catalog(c: &mut Criterion) {
    c.bench_function("JSON lex of citm_catalog", |b| b.iter(citm_catalog));
}
fn benchmark_twitter(c: &mut Criterion) {
    c.bench_function("JSON lex of twitter", |b| b.iter(twitter));
}
fn benchmark_simple(c: &mut Criterion) {
    c.bench_function("JSON lex of simple", |b| b.iter(simple));
}

fn benchmark_colours(c: &mut Criterion) {
    c.bench_function("JSON lex of colours", |b| b.iter(colours));
}
criterion_group! {
    name = benches;
